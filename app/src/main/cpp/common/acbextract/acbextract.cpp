...
AcbWalkOptions::AcbWalkOptions()
        : decoderConfig({}), useCueName(FALSE), discovery(ACB_EXTRACT_DISCOVER_DEFAULT),
          prependId(FALSE), callback(nullptr) {
}

static int ProcessAllBinaries(CAcbFile &acb, uint32_t formatVersion, AcbWalkOptions *options,
                              const string &extractDir, const CAfs2Archive *archive,
                              IStream *archiveDataStream, bool_t isInternal) {
...
int AcbWalk(const std::string &inputAcbFile, const std::string &outputDirectory,
            AcbWalkOptions *options) {
    const auto baseExtractDirPath = CPath::Combine(outputDirectory,"_acb_" + CPath::GetFileName(inputAcbFile));
    ...
    if (archive) {
        ...
        try {
            r = ProcessAllBinaries(acb, formatVersion, options, extractDir, archive,
                                   acb.GetStream(), TRUE);
        } catch (CException &ex) {
            fprintf(stderr, "%s (%d)\n", ex.GetExceptionMessage().c_str(), ex.GetOpResult());
            r = -1;
